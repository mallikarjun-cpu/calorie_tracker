...
@app.route('/view/<date>', methods=['GET', 'POST']) #date will be format of 20200310
def view(date):
    db = get_db()
...
@app.route('/view_old', methods =['POST', 'GET'], defaults={'date' : 'Default'})
@app.route('/view_old/<date>', methods=[ 'POST','GET'])
def view_old(date):
	db = get_db()
	cur = db.execute('select id, entry_date from log_date')
	results = cur.fetchall()

	return render_template('view_dates.html', display=True , results=results)
...
def connect_db():
    sql = sqlite3.connect('/home/arjun1995/calorie_tracker/sqlite3/food_log.db')
